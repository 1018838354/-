...
int tui(int *y,int k,char c3[]){
  ...
  if(k==2){c3[(*y)-10]=' ';c3[(*y)-9]=' ';
      for(m=(*y)-8;m<(*y)+2;m++)
          c3[m]=su[m-2];(*y)+=2;}
  if(k==4){for(m=(*y)-7;m<(*y)-3;m++)c3[m]=' ';
      for(m=(*y)-3;m<(*y)+4;m++)
         c3[m]=su[m-4];(*y)+=4;}
  return 0;
}
int loading(){
   int i,y,ad=0,z,h=0,x,lk=1;
...
//     }fclose(data);q();printf("数据加载成功\n");sleep(0.03);return
   ...
   if(rn<33)rt=1;//判断数据长度
     c3[0]='|';c3[1]='\0';int l2,l3,d3,l4,d4,ui;
   for(i=0,y=1;i<=rn;){
   //j();sleep(0.03);printf("%d,%d,%d,%d,%d,%d,%s\n",z,rt,rb,i,rn,y,c3);
    //一行数据扫描检测
     ...
     if(c[i]==' ' ||c[i]=='\t'){if(c[i]==' ' &&c[i+1]==' ')i++;
                       ...
                       i++;y++;}  }
     else{if(rb==1 &&c3[28]=='|'){if(z==2)l2=y-1;rb=0;}
          if(rb==1 &&c3[25]=='|'){tui(&y,2,c3);if(z==2)l2=y-1;rb=0;}
          if(rb==1 &&c3[22]=='|'){tui(&y,4,c3);if(z==2)l2=y-1;rb=0;}
          c3[y]=c[i];i++;y++;}
         }d4=y-l3;if(z==4 &&d4!=12)rt=1;
       if(z!=4 ||rt!=0 ||rb!=0){
            if(lk==1){lk++;q();printf("错误数据:\n");}
       q();printf("%s",c);sleep(2);}
      else{c3[y]='|';c3[y+1]='\n';c3[y+2]='\0';
           create(c3);h++;}  }}//一行处理完成
   if(h!=0){dawrite();q();printf("添加成功的数据:\n");
     ...
     datahead(1);
     while(1){printf("%s",a->dat);
     if(a->next==NULL)break;
     else {a=a->next;}}sleep(4);}
}
int daload(){
  if(mk!=0)return 0;
  int ad=0;
